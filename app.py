...
def get_project_data():
    if 'username' in session:
        ...
        json_file_path = os.path.join(project_path, 'data.json')

        if os.path.exists(json_file_path):
            with open(json_file_path, 'r') as f:
                data = json.load(f)
                print(f"Loaded JSON data for project '{project_name}': {data}")
        else:
            data = {}
            print(f"No JSON data found for project '{project_name}'")

        total_area = 0
        input_data = data.get('input', {})
        if input_data:
            for page_name, line_items in input_data.items():
                for line_item in line_items:
                    width = line_item['coordinates'].get('width', 0)
                    height = line_item['coordinates'].get('height', 0)
                    area = calculate_area(width, height)
                    line_item['area'] = area
                    total_area += area
        else:
            print(f"No input data found in JSON for project '{project_name}'")

        left_html = render_template('project_details.html', company_name=company_name, project_name=project_name, data=input_data, total_area=total_area)

        return jsonify({'left_html': left_html})
...
def projects(company_name):
    if 'username' in session:
        ...
        # Handle POST request for creating a new project via AJAX
        if request.method == 'POST':
            data = request.get_json()
            ...
            if project_name:
                ...
                return jsonify({'success': True})  # Return success in JSON response
            return jsonify({'success': False, 'error': 'Project name missing'}), 400

        # Handle GET request - listing projects
        projects = [p for p in os.listdir(company_path) if p != '.DS_Store' and os.path.isdir(os.path.join(company_path, p))]
        return render_template('projects.html', company_name=company_name, projects=projects)
    return redirect(url_for('login'))
